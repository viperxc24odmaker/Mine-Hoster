...
class ServerManager:
        ...
        try:
            ...
            for name, raw in data.items():
                if isinstance(raw, dict):
                    raw = dict(raw); raw.pop("running", None); self.servers[name] = ServerConfig(**raw)
    ...
    def create_server(self, config, download_url, progress_cb=None):
        ...
        folder.mkdir(parents=True, exist_ok=True); config.folder = str(folder)
        try:
            ...
            self._write_properties(folder, config); self.servers[config.name] = config; self._save()
    ...
    def _prepare_forge(self, config, folder, url, progress_cb):
        ...
        if result.returncode != 0:
            tail = "\n".join((result.stdout or "").splitlines()[-8:]); raise RuntimeError(f"Forge installer failed (exit {result.returncode}).\n{tail}")
        if not script.exists(): raise RuntimeError("Forge installed, but its run script was not generated.")
        (folder / "user_jvm_args.txt").write_text(f"-Xms{min(1024, max(512, config.ram_mb))}M\n-Xmx{max(512, config.ram_mb)}M\n", encoding="utf-8")

    def _write_properties(self, folder, cfg):
        props = {"server-port": cfg.port, "online-mode": str(cfg.online_mode).lower(), "enable-command-block": str(cfg.command_blocks).lower(), "max-players": cfg.max_players, "difficulty": cfg.difficulty, "gamemode": cfg.gamemode, "pvp": str(cfg.pvp).lower(), "white-list": str(cfg.whitelist).lower(), "motd": cfg.motd, "level-name": "world", "spawn-protection": 16, "allow-nether": "true", "allow-flight": "false", "spawn-monsters": "true", "generate-structures": "true", "view-distance": 10}
        (folder / "server.properties").write_text("#Minecraft server properties\n" + "\n".join(f"{k}={v}" for k, v in props.items()) + "\n", encoding="utf-8")

    def get_properties(self, name):
        cfg = self.servers.get(name)
        if not cfg: return {}
        result = {}; path = Path(cfg.folder) / "server.properties"
        try:
            for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
                if "=" in line and not line.startswith("#"):
                    key, _, value = line.partition("="); result[key.strip()] = value.strip()
        except OSError: pass
        return result
    ...
    def update_properties(self, name, props):
        ...
        existing = self.get_properties(name); existing.update(props)
        (Path(cfg.folder) / "server.properties").write_text("#Minecraft server properties\n" + "\n".join(f"{k}={v}" for k, v in existing.items()) + "\n", encoding="utf-8")
        return True

    def start_server(self, name):
        cfg = self.servers.get(name)
        if not cfg or self.is_running(name): return bool(cfg and self.is_running(name))
        folder = Path(cfg.folder)
        try:
            if cfg.loader == "bedrock":
                exe = folder / "bedrock_server.exe"
                if not exe.exists(): raise RuntimeError("bedrock_server.exe not found. Recreate the server.")
                cmd = [str(exe)]
            elif cfg.loader == "forge":
                script = folder / ("run.bat" if os.name == "nt" else "run.sh")
                if not script.exists(): raise RuntimeError("Forge run script not found. Recreate the server.")
                cmd = ["cmd", "/c", script.name] if os.name == "nt" else ["bash", script.name]
            else:
                jar = folder / "server.jar"
                if not jar.exists(): raise RuntimeError("server.jar not found. Recreate the server.")
                java = ensure_java(_java_required(cfg.version), lambda stage, msg, pct=None: self._emit(name, f"[MineHoster] {msg}"))
                ram = max(512, int(cfg.ram_mb))
                cmd = [java, f"-Xms{min(1024, ram)}M", f"-Xmx{ram}M", "-XX:+UseG1GC", "-XX:+ParallelRefProcEnabled", "-XX:MaxGCPauseMillis=200", "-XX:+DisableExplicitGC", "-XX:+PerfDisableSharedMem", "-jar", "server.jar", "--nogui"]
            proc = subprocess.Popen(cmd, cwd=str(folder), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding="utf-8", errors="replace", bufsize=1, env=os.environ.copy())
        except Exception as exc:
            cfg.running = False; self._emit(name, f"[ERROR] Could not start server: {exc}"); return False
        self.processes[name] = proc; cfg.running = True; self._emit(name, f"[MineHoster] Starting {cfg.loader.title()} {cfg.version}...")
        threading.Thread(target=self._read_output, args=(name, proc), daemon=True).start(); return True

    def _read_output(self, name, proc):
        try:
            if proc.stdout:
                for line in proc.stdout: self._emit(name, line.rstrip("\r\n"))
        except Exception as exc: self._emit(name, f"[ERROR] Console reader stopped: {exc}")
        finally:
            code = proc.poll()
            if code is None:
            ...
            if name in self.servers: self.servers[name].running = False
    ...
    def stop_server(self, name):
        ...
        try:
            if proc.stdin: proc.stdin.write("stop\n"); proc.stdin.flush()
        except (BrokenPipeError, OSError): pass
        def wait_and_kill():
            try: proc.wait(timeout=12)
            except subprocess.TimeoutExpired:
                self._emit(name, "[MineHoster] Stop timed out; terminating process..."); proc.terminate()
                try: proc.wait(timeout=5)
                except subprocess.TimeoutExpired: proc.kill()
        threading.Thread(target=wait_and_kill, daemon=True).start(); return True

    def restart_server(self, name):
        if self.is_running(name):
            self.stop_server(name); deadline = time.time() + 20
            while self.is_running(name) and time.time() < deadline: time.sleep(0.25)
        return self.start_server(name)

    def send_command(self, name, command):
        proc = self.processes.get(name)
        if not proc or proc.poll() is not None: self._emit(name, "[ERROR] Server is not running."); return False
        command = (command or "").strip().lstrip("/")
        if not command: return False
        try:
            if not proc.stdin: raise BrokenPipeError("stdin is unavailable")
            proc.stdin.write(command + "\n"); proc.stdin.flush(); return True
        except (BrokenPipeError, OSError) as exc: self._emit(name, f"[ERROR] Failed to send command: {exc}"); return False

    def is_running(self, name):
        proc = self.processes.get(name); return bool(proc and proc.poll() is None)

    def delete_server(self, name):
        self.stop_server(name); deadline = time.time() + 15
        while self.is_running(name) and time.time() < deadline: time.sleep(0.2)
        cfg = self.servers.pop(name, None); self.console_callbacks.pop(name, None)
        if cfg and cfg.folder: shutil.rmtree(cfg.folder, ignore_errors=True)
        self._save()
    ...
    def unregister_console_callbacks(self, name): self.console_callbacks.pop(name, None)
    ...
    def _json_add(self, name, filename, entry):
        ...
        if not any(item.get("name") == entry.get("name") for item in values): values.append(entry)
    ...
    def add_plugin(self, name, jar_path):
        directory = self.get_plugins_dir(name)
        if directory: directory.mkdir(parents=True, exist_ok=True); shutil.copy2(jar_path, directory / Path(jar_path).name)
    def remove_plugin(self, name, plugin_name):
        directory = self.get_plugins_dir(name)
        if directory and (directory / plugin_name).exists(): (directory / plugin_name).unlink()

    def _safe_path(self, name, rel_path):
        cfg = self.servers.get(name)
        if not cfg: return None
        root = Path(cfg.folder).resolve(); target = (root / rel_path).resolve()
        try: target.relative_to(root)
        except ValueError: return None
        return target
    def list_files(self, name, subpath=""):
        cfg = self.servers.get(name); base = self._safe_path(name, subpath)
        if not cfg or not base or not base.exists(): return []
        root = Path(cfg.folder).resolve()
        return [{"name": p.name, "is_dir": p.is_dir(), "size": p.stat().st_size if p.is_file() else 0, "path": str(p.relative_to(root))} for p in sorted(base.iterdir(), key=lambda p: (not p.is_dir(), p.name.lower()))]
    def read_file(self, name, rel_path):
        target = self._safe_path(name, rel_path)
        if not target: return ""
        try: return target.read_text(encoding="utf-8", errors="replace")
        except OSError: return ""
    def write_file(self, name, rel_path, content):
        target = self._safe_path(name, rel_path)
        if not target: return False
        target.parent.mkdir(parents=True, exist_ok=True); target.write_text(content, encoding="utf-8"); return True
