...
class PluginsView:
    ...
    def _loader(self):
        ...
        if value != "Auto":
            return value
        cfg = self._cfg()
        return cfg.loader if cfg and cfg.loader in ("paper", "purpur", "spigot", "bukkit", "fabric", "forge", "neoforge") else ""
    ...
    def _install_project(self, project_id, name, minecraft_version, loader):
        if not self.selected:
            return
        self.status_text.value = f"Checking dependencies for {name}..."
        self.progress.visible = True
        ...
        def worker():
            try:
                ...
                if filename.lower() in existing:
                    ...
                    self.status_text.value = f"Installed {name} ({len(files)} file(s), dependencies checked)."
                    self.status_text.color = COLORS["accent2"]
