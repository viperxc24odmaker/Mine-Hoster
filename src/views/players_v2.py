...
class PlayersViewV2:
    def __init__(self,app):
        self.app=app;self.sm=ServerManager.get();self.selected=app.selected_server or (self.sm.get_servers()[0].name if self.sm.get_servers() else None);self.tab="online";self.online=[];self.rows=ft.Column(spacing=8,expand=True,scroll=ft.ScrollMode.AUTO);self.status=ft.Text("",color=COLORS["subtext"],size=12);self.input=ft.TextField(label="Player name",width=240)
        if self.selected:self.sm.register_console_callback(self.selected,self._console)
    def build(self):
        servers=self.sm.get_servers();dd=ft.Dropdown(value=self.selected,options=[ft.dropdown.Option(s.name) for s in servers],on_change=self._server,width=220);tabs=ft.Tabs(selected_index=["online","whitelist","ops","banned"].index(self.tab),on_change=self._tab,tabs=[ft.Tab(text="Online"),ft.Tab(text="Whitelist"),ft.Tab(text="Operators"),ft.Tab(text="Banned")]);self._refresh();return ft.Container(content=ft.Column([ft.Row([ft.Text("Players",size=26,weight=ft.FontWeight.BOLD,color=COLORS["text"]),dd]),tabs,ft.Row([self.input,ft.ElevatedButton("Add",on_click=self._add),ft.ElevatedButton("Refresh",on_click=self._query)]),self.status,self.rows],expand=True),padding=28,expand=True)
    def _console(self,line):
        m=re.search(r"There are\s+\d+\s+of a max(?:imum)?\s+\d+\s+players online:\s*(.*)$",line,re.I) or re.search(r"players online:\s*(.*)$",line,re.I)
        if m:self.online=[x.strip() for x in m.group(1).split(",") if x.strip() and x.strip().lower() not in ("none","-")];self._render()
    ...
    def _refresh(self):
        ...
        for name,action in values:
            btn=ft.ElevatedButton(action,disabled=action=="Online",on_click=lambda e,n=name:self._remove(n));self.rows.controls.append(ft.Container(content=ft.Row([ft.Text("●",color=COLORS["accent2"] if action=="Online" else COLORS["muted"]),ft.Text(name,color=COLORS["text"],expand=True),btn]),bgcolor=COLORS["card"],border_radius=10,padding=12))
