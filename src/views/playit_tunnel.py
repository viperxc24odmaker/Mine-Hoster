...
class PlayitTunnelView:
    def __init__(self,app):
        self.app=app;self.sm=ServerManager.get();self.playit=PlayitManager.get();self.selected=app.selected_server or (self.sm.get_servers()[0].name if self.sm.get_servers() else None);self.status=ft.Text("Ready.",color=COLORS["subtext"],size=12);self.address=ft.Text("",color=COLORS["accent2"],size=15,selectable=True,weight=ft.FontWeight.BOLD);self.claim=ft.Text("",color=COLORS["warning"],size=12,selectable=True);self.log=ft.Text("",color=COLORS["muted"],size=10,selectable=True);self.progress=ft.ProgressBar(visible=False,value=0,color=COLORS["accent"]);self.setup_code=ft.TextField(label="Optional setup code",hint_text="Leave blank for the official claim flow",expand=True)
    def _card(self,t,s,c):return ft.Container(content=ft.Column([ft.Text(t,size=16,weight=ft.FontWeight.BOLD,color=COLORS["text"]),ft.Text(s,size=12,color=COLORS["subtext"]),ft.Container(height=6),*c],spacing=8),bgcolor=COLORS["card"],border_radius=14,padding=20,border=ft.border.all(1,COLORS["border"]))
    def build(self):
        ...
        return ft.Container(content=ft.Column([ft.Text("Playit.gg",size=26,weight=ft.FontWeight.BOLD,color=COLORS["text"]),ft.Text("Free public tunneling for local Minecraft servers — no port forwarding.",size=13,color=COLORS["subtext"]),ft.Container(height=10),self._card("Tunnel target","The local origin is always 127.0.0.1 and the selected server port.",[dd,ft.Row([ft.ElevatedButton("Install / Update Agent",on_click=self._install),ft.ElevatedButton("First-time Setup",on_click=self._setup),ft.ElevatedButton("Connect Agent",on_click=self._connect),ft.ElevatedButton("Stop Agent",on_click=self._stop)],wrap=True),self.setup_code,self.progress,self.status]),ft.Container(height=14),self._card("Create a free Minecraft tunnel","Playit requires the account-side tunnel to be created after the agent is claimed. MineHoster opens the official wizard with the correct local port ready to enter.",[ft.Row([ft.ElevatedButton("Create Free Tunnel",on_click=self._create_tunnel),ft.ElevatedButton("Open Tunnel Dashboard",on_click=lambda e:webbrowser.open("https://playit.gg/account/tunnels"))],wrap=True),self.address,self.claim]),ft.Container(height=14),self._card("Agent output","Live diagnostics and public address detection.",[self.log])],scroll=ft.ScrollMode.AUTO),padding=28,expand=True)
    def _safe(self):
