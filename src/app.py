...
def _ensure_server_assets(manager, cfg):
    ...
    if cfg.loader == 'bedrock' and not (folder / 'bedrock_server.exe').exists():
        ...
        if not jar.exists() or jar.stat().st_size < 1024:
            ...
            try:
                manager._download(url, part, lambda kind, message, percent=None: manager._emit(cfg.name, f'[Download] {message}')); part.replace(jar)
...
def _start_server_with_auto_setup(self, name):
    ...
    try: _ensure_server_assets(self, cfg)
        ...
        self._emit(name, f'[ERROR] Server setup failed: {exc}'); return False
        ...
        while time.monotonic() < deadline:
            if self.is_running(name):
                ...
                while time.monotonic() < stable_until:
                    if not self.is_running(name): break
                    time.sleep(0.1)
                if self.is_running(name): return True
                break
            time.sleep(0.1)
        if attempt == 0:
            self._emit(name, '[MineHoster] Server exited during first startup; retrying once automatically...'); time.sleep(0.5)
...
class MineHosterApp:
    def __init__(self, page: ft.Page):
        self.page = page; self.current_view = 'dashboard'; self.selected_server = None; self.nav_refs = {}; self._main_content = None; self.starting_servers = set(); self._navigation_token = 0; self._navigation_lock = threading.Lock()

    def initialize(self):
        page = self.page; page.title = 'MineHoster'; page.bgcolor = COLORS['bg']; page.padding = 0; page.spacing = 0; page.window_width = 1320; page.window_height = 820; page.window_min_width = 1000; page.window_min_height = 660
        try:
    ...
    def _build_sidebar(self):
        ...
        return ft.Container(content=ft.Column([logo, ft.Container(height=1, bgcolor=COLORS['border']), ft.Container(content=ft.Column(buttons, spacing=3), padding=ft.padding.symmetric(horizontal=9, vertical=13)), ft.Container(expand=True), ft.Container(content=ft.Column([ft.Text('MINEHOSTER', color=COLORS['muted'], size=9, weight=ft.FontWeight.BOLD), ft.Text('Graphite • Local-first • No cloud required', color=COLORS['subtext'], size=10)], spacing=2), padding=ft.padding.only(left=18, bottom=18))], spacing=0, expand=True), bgcolor=COLORS['surface'], width=238, border=ft.border.only(right=ft.BorderSide(1, COLORS['border'])))

    def _nav_button(self, key, icon, label):
        active = key == self.current_view
        return ft.TextButton(content=ft.Row([ft.Text(icon, color=COLORS['text'] if active else COLORS['muted'], size=16, weight=ft.FontWeight.BOLD), ft.Text(label, color=COLORS['text'] if active else COLORS['subtext'], size=12, weight=ft.FontWeight.W_500)], spacing=12), style=ft.ButtonStyle(padding=ft.padding.symmetric(horizontal=12, vertical=11), alignment=ft.alignment.center_left, shape=ft.RoundedRectangleBorder(radius=9), bgcolor=COLORS['accent_soft'] if active else None), on_click=lambda e, selected=key: self.navigate(selected))

    def _set_nav_state(self, view_key):
        self.current_view = view_key
        for key, button in self.nav_refs.items():
            active = key == view_key; row = button.content; row.controls[0].color = COLORS['text'] if active else COLORS['muted']; row.controls[1].color = COLORS['text'] if active else COLORS['subtext']; button.style.bgcolor = COLORS['accent_soft'] if active else None
