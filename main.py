...
from src.views.plugins import PluginsView as _PluginsView
...
def _cached_ensure_java(required, progress_cb=None):
    ...
    for root in runtime_dir.glob("temurin-*"):
        ...
        try:
            result=subprocess.run([str(java),"-version"],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,timeout=5,check=False,creationflags=getattr(subprocess,"CREATE_NO_WINDOW",0) if os.name=="nt" else 0); output=result.stdout or ""; marker='version "'
            if marker in output:
...
_original_plugin_init=_PluginsView.__init__
def _plugin_init(self,app):
    _original_plugin_init(self,app)
    self.loader_field.options=[ft.dropdown.Option(v) for v in ["Auto","paper","fabric","forge","neoforge"]]
    if self.loader_field.value not in {"Auto","paper","fabric","forge","neoforge"}:self.loader_field.value="Auto"
_PluginsView.__init__=_plugin_init
...
def _build_sidebar(self):
    ...
    items=[("dashboard","⌂","Overview"),("bedrock","◇","Bedrock"),("create","+","Create Server"),("console",">_","Console"),("files","□","Files"),("plugins","◆","Plugins & Mods"),("players","●","Players"),("playit","↗","Playit.gg"),("settings","⚙","Settings"),("hosting","◈","Hosting")];buttons=[]
    for key,icon,label in items:b=self._nav_button(key,icon,label);self.nav_refs[key]=b;buttons.append(b)
...
def _navigate(self,view_key):
    ...
    except Exception as exc:self._main_content.content=ft.Container(content=ft.Column([ft.Text("Section failed to load",size=22,weight=ft.FontWeight.BOLD,color=COLORS["text"]),ft.Text(str(exc),color=COLORS["danger"],selectable=True),ft.ElevatedButton("Back to Overview",on_click=lambda e:self.navigate("dashboard"))],spacing=12),padding=30,expand=True);self.page.update()
